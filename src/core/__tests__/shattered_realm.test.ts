...
describe('Shattered Realm (Sındırılmış Dünya) Campaign Scenario', () => {
  it('correctly initializes shattered realm with exactly 9 player capitals and 88 neutral baronies', () => {
    ...
    for (let i = 0; i < game.regionState.length; i++) {
      ...
      if (rSt.owner === -1) {
        ...
        // Starting capital has starting troops (5)
        expect(rSt.troops).toBe(RULES.startingTroops);
      }
    }

    expect(ownedCount).toBe(9);
    expect(neutralCount).toBe(88);
  ...
  it('allows human player to conquer adjacent neutral baronies', () => {
    ...
    // March 4 troops to conquer neutral barony (which has 2 troops)
